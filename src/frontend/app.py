...
with st.expander("📚 詳しい説明を読む", expanded=False):
    ...
    **例:**
    ...
    単一の防御策に頼るのではなく、**複数の防御層**を組み合わせることで、攻撃を阻止または検知します。
    
    #### 🔴 第1層: LLMレベルの防御（ガードレール）
    ...
    - **手法**: READ_ONLY権限では送金や口座変更ができない
    - **効果**: AIが騙されても、システムが物理的に実行を拒否
    
    #### 🟢 第3層: 検知レイヤー（監査システム）
    - **目的**: 攻撃が成功しても、事後的に必ず発見する
    - **手法**: 取引ログを自動スキャンし、異常を検出
    - **効果**: 被害の早期発見と拡大防止
    ...
    1. **サイドバーで権限を選択**
       ...
       - `READ_ONLY`: 書き込み操作は禁止（システムが防御）
    
    2. **タブを切り替えて実験**
       - 🔴 **脆弱なエージェント**: 防御なし（攻撃の脅威を実証）
       - 🟢 **堅牢なエージェント**: ガードレール付き（LLM防御を実証）
       - 👮 **銀行監査システム**: 事後検知（検知層を実証）
...
# Tabs
tab1, tab2, tab3 = st.tabs(["🔴 脆弱なエージェント (攻撃デモ)", "🟢 堅牢なエージェント (防御デモ)", "👮 銀行監査システム"])
